...
from nose.tools import (assert_equal, assert_greater, assert_in, assert_not_in, assert_multi_line_equal)
from nose.plugins.skip import SkipTest

from semanticizest.parse_wikidump.__main__ import main as parse_wikidump_main
from semanticizest.parse_wikidump import (page_statistics, parse_dump, clean_text, extract_links, remove_links)
...
def test_extract_links_broken():
    raise SkipTest('these need to be sorted out')
...
def test_page_statistics():
    raise SkipTest("still fails on '[[statistic]]s'")
    page = """
...
def test_remove_links():
    raise SkipTest('fails to insert a space between the links [[text]][[book]]')
    text = """
        Wikisyntax is the [[syntax (to be parsed)|syntax]] used on
        [[Wikipedia]].{{citation needed|date=October 2014}}
        We have to parse it, and we use every [[hack]] in the
    ...
    expected_str = "\n".join(expected.split())
    actual_str = "\n".join(remove_links(clean_text(text)).split())
    assert_multi_line_equal(actual_str, expected_str)
